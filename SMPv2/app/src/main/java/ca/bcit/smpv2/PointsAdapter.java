    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(curTier != UserPointsActivity.tiers.size() - 1) {
            ...
            ((TextView) convertView.findViewById(R.id.pointsTextView))
                    .setText(String.valueOf(userPoints.second.getPoints()) + " points");

            ((TextView) convertView.findViewById(R.id.curTier))
                    .setText(UserPointsActivity.tiers.get(curTier).getName() + " (" + UserPointsActivity.tiers.get(curTier).getMinPoints() + ")");
            ((TextView) convertView.findViewById(R.id.nextTier))
                    .setText(UserPointsActivity.tiers.get(curTier + 1).getName() + " (" + UserPointsActivity.tiers.get(curTier + 1).getMinPoints() + ")");
        }else{
            seekBar.setProgress(100);
            ((TextView) convertView.findViewById(R.id.pointsTextView))
                    .setText(String.valueOf((int)((float) seekBar.getProgress() / 100 * UserPointsActivity.tiers.get(curTier).getMinPoints())) + " points");
